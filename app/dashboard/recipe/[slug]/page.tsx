...
const RecipeDetailPage = () => {
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    retry: 1 // Limit retries for 404s
  });
            ...
            <div className="flex items-center text-sm text-gray-400">
              <div className="flex items-center">
                <div className="w-8 h-8 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 flex items-center justify-center text-white text-xs font-bold">
                  {recipe?.adminDetails ? recipe?.adminDetails?.name.charAt(0).toUpperCase() : 'C'}
                </div>
                <span className="ml-2">By {recipe?.adminDetails?.name || 'Chef'}</span>
              </div>
              ...
              <div className="mb-6 pb-6 border-b border-gray-700">
                <div className="flex items-center mb-4">
                  ...
                  <span className="text-white font-medium">
                    {recipe.averageRating?.toFixed(1) || "No ratings"} {recipe.averageRating ? "out of 5" : ""}
                  </span>
                </div>
