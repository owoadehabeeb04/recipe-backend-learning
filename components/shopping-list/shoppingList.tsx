...
const ShoppingList: React.FC<ShoppingListProps> = ({ mealPlanId }) => {
  ...
  const [shoppingData, setShoppingData] = useState<ShoppingListData | null>(
    null
  );
    ...
    try {
      ...
      if (item) {
        ...
        if (updatedData.categoryStats && updatedData.categoryStats[category]) {
          updatedData.categoryStats[category].checked += !isCurrentlyChecked
            ? 1
            : -1;
        }
  ...
  const toggleAllInCategory = async (category: string) => {
    ...
    try {
      ...
      // Update category stats
      if (updatedData.categoryStats && updatedData.categoryStats[category]) {
        updatedData.categoryStats[category].checked = allChecked
          ? 0
          : items.length;
      }
  ...
  const handleResetCheckedItems = async () => {
    ...
    if (
      window.confirm(
        "Are you sure you want to reset your shopping list progress?"
      )
    ) {
      try {
        setIsSaving(true);
        ...
        Object.keys(updatedData.categorizedIngredients).forEach((category) => {
          ...
          // Update category stats
          if (
            updatedData.categoryStats &&
            updatedData.categoryStats[category]
          ) {
  ...
  const calculateProgress = useCallback(() => {
    ...
    Object.entries(shoppingData.categorizedIngredients).forEach(
      ([category, items]) => {
                ...
                ${items
                  ...
                  <li class="${item.checked ? "checked" : ""}">
                    ...
                    <div class="item-detail">
                      ${item.items
                        .map((subItem) => `${subItem.quantity} ${subItem.unit}`)
                        .join(", ")}
                    </div>
  ...
  if (isLoading) {
    return (
      <div className="p-8 flex flex-col items-center justify-center h-64 bg-card backdrop-blur-sm border border-border rounded-xl">
  ...
  if (error) {
    return (
      <div className="p-6 bg-card backdrop-blur-sm border border-destructive/30 rounded-xl">
        ...
        <button
          ...
          className="px-4 py-2 bg-primary/20 hover:bg-primary/30 text-foreground rounded-lg border border-primary/30 flex items-center"
        >
  ...
  if (
    !shoppingData ||
    !shoppingData.categorizedIngredients ||
    Object.keys(shoppingData.categorizedIngredients).length === 0
  ) {
    return (
      <div className="p-8 bg-card backdrop-blur-sm border border-border rounded-xl text-center">
        <ShoppingBag className="w-12 h-12 text-primary mx-auto opacity-50 mb-4" />
        <h3 className="text-xl font-medium text-foreground mb-2">
          No ingredients found
        </h3>
        <p className="text-muted-foreground mb-6">
          This meal plan doesn't have any recipes with ingredients.
        </p>
        <button
          onClick={fetchShoppingList}
          className="px-4 py-2 bg-primary/20 hover:bg-primary/30 text-foreground rounded-lg border border-primary/30 flex items-center mx-auto"
        >
          <RefreshCw className="w-4 h-4 mr-2" />
          Refresh
        </button>
  ...
  return (
    <div
      className={`bg-card mt-3 backdrop-blur-sm border border-border rounded-xl overflow-hidden ${isSaving ? "opacity-70 pointer-events-none" : ""}`}
    >
      {isSaving && (
      ...
      <div className="bg-primary/10 p-4">
            ...
            <p className="text-xs text-muted-foreground">
              ...
              {shoppingData.lastUpdated && (
                <span className="ml-2 opacity-60">
                  • Updated{" "}
                  {format(new Date(shoppingData.lastUpdated), "MMM d, h:mm a")}
                </span>
              )}
            </p>
          </div>
        ...
        {/* Progress bar */}
        <div className="mt-3 mb-1 bg-muted h-2 rounded-full overflow-hidden">
          <div
      ...
      <div className="p-4 space-y-4">
        {Object.entries(shoppingData.categorizedIngredients).map(
          ([category, items]) => {
            // Get category stats
            const stats = shoppingData.categoryStats?.[category] || {
              total: items.length,
              checked: 0
            };
            ...
            return (
              <div
                ...
                // opacity: 1, y: 0 }}
                // duration: 0.2 }}
                className="bg-primary/5 rounded-xl border border-border overflow-hidden"
              >
                {/* Category header */}
                <div className="flex items-center justify-between bg-primary/10 px-4 py-3">
                  ...
                  >
                    ...
                    <span className="ml-2 text-xs bg-primary/20 px-2 py-0.5 rounded-full text-primary">
                      {items.length}
                    </span>
                    {stats.checked > 0 && (
                      <span className="ml-2 text-xs bg-primary/30 px-2 py-0.5 rounded-full text-primary">
      ...
      {progress === 0 && (
        <div className="px-4 pb-4">
          ...
          >
            ...
            <p className="text-xs text-muted-foreground">
              Shop by category to save time! Start at the top and work your way
              down.
            </p>
          </div>
        </div>
